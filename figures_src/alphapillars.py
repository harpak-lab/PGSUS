...
class alphapillars(object):
	...
	def plot_wc(self):
		for label in ['1kg.all', '1kg.eur']:	
			...
			if label == '1kg.all':
				ordering = alpha_dfplot.index.tolist()
			alpha_dfplot = alpha_dfplot.loc[ordering]
			...
			fig, ax = plt.subplots(nrows = 2, ncols = 4, figsize = (10,10), \
						gridspec_kw = {'width_ratios' :[4,0.25,4,0.25,]})
			for i,j in zip([[0,0],[0,2],[1,0],[1,2]],alpha_dfplot.columns):
				ax[i[0],i[1]].vlines(1, ymin = 0.5, ymax =18.5, linestyles='dashed', color = 'black')
				ax[i[0],i[1]].errorbar(alpha_dfplot[j],alpha_dfplot['ycoordinate'], xerr = alpha_se_df[j],color = '#CA6627', linestyle = '', capsize = 3)
				ax[i[0],i[1]].scatter(alpha_dfplot[j],alpha_dfplot['ycoordinate'],color = '#CA6627', s = 45)
				ax[i[0],i[1]].title.set_text('p < ' + str(j))
				ax[i[0],i[1]].set_axisbelow(True)
				ax[i[0],i[1]].set_xlabel(r'$\hat{\alpha}$')
				ax[i[0],i[1]].grid(True)
				if i[1] == 0:
					ax[i[0],i[1]].set_yticks(alpha_dfplot['ycoordinate'])
					ax[i[0],i[1]].set_yticklabels([self.label_dict[trait] for trait in alpha_dfplot.index.tolist()])
				else:
					ax[i[0],i[1]].set_yticks([x+1 for x in range(alpha_dfplot.shape[0])])
					ax[i[0],i[1]].set_yticklabels(['' for x in range(alpha_dfplot.shape[0])])
				ax[i[0],i[1]+1].set_ylim(ax[i[0],i[1]].get_ylim())
				ax[i[0],i[1]+1].axis('off')
				for k in alpha_dfplot.index.tolist():
					ax[i[0],i[1]+1].text(-3,alpha_dfplot.loc[k,'ycoordinate'] ,str(nsnp.loc[k][j]) + ' SNPs', verticalalignment = 'center')
			sns.despine()
			plt.tight_layout()
