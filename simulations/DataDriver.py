...
from component_error import component_error
...
parser.add_argument('--correct', default=True, action=argparse.BooleanOptionalAction, dest = 'correct')
...
if args.sim_dir:
	...
	params = {'nsims':args.nsim, 'ngwaspcs':args.ngwaspcs, 'nanc':args.nanc, 'nloci':args.nloci, 'n_trait_loci':args.n_trait_loci, 
			  ...
			  'env_covariance':args.env_covariance, 'ascp':args.ascp, 'pc_lower_bound':args.pc_lower_bound, 'correct':args.correct, 
			  'neutral_architecture':args.neutral_architecture}
	
	sim_params_manifest = pd.DataFrame.from_dict(params, orient = 'index')
	sim_params_manifest.to_csv('simulation_cache/sim.params.' + args.outlabel + '.txt',sep = '\t', index = False)
	...
	print("Simulations for the following parameters:\n\
		...
		".format(args.nsim, args.ngwaspcs, args.nanc, args.nloci, args.n_trait_loci, args.n_standard_gwas, args.n_sib_gwas, 
			...
			args.drift_param, args.env_variance, args.env_covariance, args.ascp, args.pc_lower_bound, args.correct, args.neutral_architecture))

	if os.path.exists('simulation_cache/' + args.outlabel):
		pass 
	else:
		os.system('mkdir simulation_cache/' + args.outlabel)
	...
	for sim in range(1,args.nsim+1):
		...
		simulation_gwas = GWAS(nloci=args.nloci, nanc=args.nanc, n_standard_gwas = args.n_standard_gwas, n_sib_gwas = args.n_sib_gwas, n_target_cohort = args.n_target_cohort,
				...
				gwas_pcs = args.ngwaspcs, correct=args.correct, boot = False, neutral_architecture = args.neutral_architecture, covs = None)
		
		simulation_gwas = simulation_gwas.outputs()

		with open('simulation_cache/' + args.outlabel + '/sim.' + simlabel + '.gwas.pkl', 'wb') as handle:
		...
		saddf = decomposition.outputs()
		with open('simulation_cache/' + args.outlabel + '/sim.' + simlabel + '.sad.pkl', 'wb') as handle:
			pickle.dump(saddf, handle, protocol=pickle.HIGHEST_PROTOCOL)
