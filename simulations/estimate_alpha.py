...
class SAD(object):
	...
	def prs_decomp_unsq(self,eff_sizes,eigenvalues,eigenvectors):
		...
		temp = eigenvalues*(eigenvectors@eff_sizes)
		return temp
	...
	def estimate_components_and_alpha(self,gwas_beta,gwas_se,sib_beta,sib_se,ascertainment_p,thresh,eigenvectors,eigenvalues,
		...
		self.decomp_gwas, self.decomp_gwas_se, self.decomp_sib, self.decomp_sib_se = decomp_gwas, decomp_gwas_se, decomp_sib, decomp_sib_se
		...
		variance_standard_vc = 2*(decomp_gwas_se**2)
		variance_sib_vc = 2*(decomp_sib_se**2)

		indices = [i for i in range(pc_lower_bound,pc_upper_bound)]
		startdf = np.vstack((standard_variance_component,direct_variance_component,variance_sad_vc,variance_direct_vc))
		...
		self.decomp_true_gwas, self.decomp_true_gwas_se, self.decomp_true_sib, self.decomp_true_sib_se = decomp_true_gwas, decomp_true_gwas_se, decomp_true_sib, decomp_true_sib_se
	...
	def outputs(self):
		return {'direct_vc':self.direct_variance_component,'sad_vc':self.sad_variance_component, 'covar_vc':self.covar_variance_component, 
				...
				'lmdf':self.lmdf,'alpha_ols':self.ols_alpha, 'decomp_true_gwas':self.decomp_true_gwas, 'decomp_true_sib':self.decomp_true_sib, 
				'decomp_true_gwas_se':self.decomp_true_gwas_se, 'decomp_true_sib_se':self.decomp_true_sib_se,'decomp_gwas_se':self.decomp_gwas_se, 'decomp_sib_se':self.decomp_sib_se}
